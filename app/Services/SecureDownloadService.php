...
class SecureDownloadService
{
    ...
    {
        abort_unless($order->status === Order::STATUS_PAID, 403, 'Order belum dibayar atau belum di-approve.');

        abort_unless($order->download_token && hash_equals($order->download_token, $token), 403, 'Token download tidak valid.');
