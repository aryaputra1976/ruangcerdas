                ...
                <div class="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm md:p-8">
                    ...
                    <form method="POST" action="{{ route('checkout.store', $product->slug) }}" class="space-y-6" onsubmit="window.rcTrack && window.rcTrack('Lead', {source: 'checkout_form', content_type: 'product', content_ids: [{{ $product->id }}], value: {{ (int) $price }}, currency: 'IDR'});">
                        ...
                        <div class="rounded-2xl border border-blue-100 bg-blue-50 px-4 py-4 text-sm leading-6 text-blue-900">
                            <p class="font-bold">Info Pengiriman Akses</p>
                            <p class="mt-1">Produk berupa file digital. Tidak ada pengiriman fisik. Link download dikirim setelah pembayaran disetujui admin.</p>
                        </div>

                        <div class="rounded-2xl border border-amber-100 bg-amber-50 px-4 py-4 text-sm leading-6 text-amber-900">
                            <p class="font-bold">Keamanan Data Pembelian</p>
                            <ul class="mt-2 list-disc space-y-1 pl-5">
                                <li>Email harus benar karena link download dikirim ke email pembeli.</li>
                                <li>Simpan nomor invoice setelah checkout untuk upload bukti bayar dan cek order.</li>
                            </ul>
                        </div>
