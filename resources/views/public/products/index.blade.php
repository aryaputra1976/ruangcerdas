        ...
        <div class="grid gap-10 lg:grid-cols-2 lg:items-center">
            <div>
                ...
                <p class="mt-5 max-w-2xl text-base leading-8 text-slate-300 md:text-lg">
                    Pilih produk digital siap pakai untuk mempercepat pekerjaan administrasi, dokumen kantor,
                    template, dan produktivitas berbasis AI.
                </p>

                <div class="mt-8 flex flex-wrap gap-3">
                    <a href="#katalog"
                       class="inline-flex items-center justify-center rounded-2xl bg-blue-600 px-6 py-3 text-sm font-bold text-white shadow-lg shadow-blue-600/30 transition hover:bg-blue-700">
                        Lihat Katalog
                    </a>

                    <a href="{{ route('home') }}"
                       class="inline-flex items-center justify-center rounded-2xl border border-white/15 bg-white/10 px-6 py-3 text-sm font-bold text-white transition hover:bg-white/15">
                        Kembali ke Beranda
                    </a>
                </div>
            </div>
    ...
    <div class="mx-auto max-w-7xl px-6">
        ...
        <div class="mb-8 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
            <div>
                ...
                <h2 class="mt-2 text-3xl font-black tracking-tight text-slate-950 md:text-4xl">
                    Produk Tersedia
                </h2>

                <p class="mt-3 max-w-2xl text-slate-600">
                    Gunakan filter untuk menemukan produk yang sesuai kebutuhan.
                </p>
            </div>
            ...
            <form method="GET" action="{{ route('products.index') }}" class="grid gap-4 lg:grid-cols-12">
                ...
                <div class="lg:col-span-6">
                    ...
                    <input
                        ...
                        name="q"
                        value="{{ request('q') }}"
                        placeholder="Cari nama, deskripsi, atau isi produk..."
                        class="w-full rounded-2xl border border-slate-300 px-4 py-3 text-slate-700 shadow-sm focus:border-blue-600 focus:ring-blue-600"
                    >
                </div>
            ...
            @if ($hasFilter)
                <div class="mt-5 flex flex-wrap items-center justify-between gap-3 rounded-2xl bg-slate-50 px-4 py-3">
                    <div class="text-sm text-slate-600">
                        ...
                        @if (request('q'))
                            <span class="ml-1 rounded-full bg-blue-100 px-3 py-1 text-xs font-bold text-blue-700">
                                Keyword: {{ request('q') }}
                            </span>
                        @endif

                        @if (request('category'))
                            <span class="ml-1 rounded-full bg-emerald-100 px-3 py-1 text-xs font-bold text-emerald-700">
        ...
        @else
            <div class="rounded-[2rem] border border-dashed border-slate-300 bg-white p-12 text-center shadow-sm">
                ...
                <p class="mx-auto mt-3 max-w-md text-slate-600">
                    ...
                    @else
                        Belum ada produk aktif dan publish di katalog Ruang Cerdas.
                    @endif
                </p>
                ...
                <div class="mt-6 flex justify-center gap-3">
                    @if ($hasFilter)
                        <a href="{{ route('products.index') }}"
                           class="inline-flex items-center justify-center rounded-2xl border border-slate-300 bg-white px-6 py-3 text-sm font-bold text-slate-700 hover:border-blue-600 hover:text-blue-600">
                            Reset Filter
                        </a>
                    @endif

                    <a href="{{ route('home') }}"
                       class="inline-flex items-center justify-center rounded-2xl bg-blue-600 px-6 py-3 text-sm font-bold text-white hover:bg-blue-700">
                        Kembali ke Beranda
                    </a>
                </div>
            </div>
